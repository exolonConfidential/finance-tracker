...
export default function SignupPage() {
  ...
  return (
    <div className="relative flex min-h-screen w-full bg-background text-foreground overflow-hidden selection:bg-primary/30">
      ...
      <div className="relative z-10 flex w-full flex-col justify-center px-8 sm:w-[500px] lg:w-[600px] bg-card/40 backdrop-blur-2xl border-r border-white/5 shadow-2xl xl:px-16 overflow-y-auto">
        {/* Back Button */}
        ...
        <motion.div
          className="mx-auto w-full max-w-[380px] py-20"
        ...
        >
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                    ...
                    <FormItem>
                      ...
                      <FormControl>
                        <Input
                          placeholder="John"
                          {...field}
                          className="h-12 bg-background/50 backdrop-blur-sm border-white/10 focus-visible:ring-primary focus-visible:border-primary transition-all"
                        />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                    ...
                    <FormItem>
                      ...
                      >
                        <FormControl>
                          ...
                          <SelectTrigger className="h-12 bg-background/50 backdrop-blur-sm border-white/10 focus:ring-primary focus:border-primary transition-all">
              ...
              {error && (
                <motion.div
                  variants={itemVariants}
                  className="rounded-xl bg-destructive/10 p-3 text-sm text-destructive border border-destructive/20 backdrop-blur-sm text-center"
                >
                  {error}
                </motion.div>
              )}
              ...
              <motion.div variants={itemVariants} className="pt-2">
                <Button
                  type="submit"
                  className="h-12 w-full bg-primary hover:bg-primary/90 text-primary-foreground font-medium shadow-[0_0_20px_rgba(79,70,229,0.3)] hover:shadow-[0_0_30px_rgba(79,70,229,0.5)] transition-all rounded-xl"
                  disabled={isLoading}
                >
                  {isLoading ? (
                    <>
          ...
          <motion.div
            variants={itemVariants}
            className="mt-8 text-center text-sm pb-10"
          >
          ...
          <div className="flex items-center gap-4 mt-8">
            <div className="flex -space-x-3">
              {[1, 2, 3, 4].map((i) => (
                <div
                  key={i}
                  className={`h-10 w-10 rounded-full border-2 border-background bg-muted flex items-center justify-center shadow-sm z-[${10 - i}]`}
                >
            ...
            <p className="text-sm font-medium text-muted-foreground">
              Join <span className="text-foreground">10,000+</span> users
              tracking wealth.
            </p>
          </div>
        </motion.div>
